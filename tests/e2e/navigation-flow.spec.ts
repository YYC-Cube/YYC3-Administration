...
test.describe('E2E-NAV: 导航流程', () => {
  ...
  test('E2E-NAV-001: 仪表盘页面加载', async ({ page }) => {
    // 验证仪表盘可见
    await expect(page.locator('[data-testid="app-container"]').or(page.locator('body'))).toBeVisible()
    // 验证导航栏存在
  ...
  test('E2E-NAV-004: 导航分类展开/折叠', async ({ page }) => {
    // 点击分类标题展开/折叠
    const category = page.locator('[data-cat-id="customer"]').or(page.locator('text=客户管理').first())
...
test.describe('E2E-NAV: 响应式', () => {
  test('E2E-NAV-006: 移动端导航', async ({ page, isMobile }) => {
    ...
    // 移动端应该有汉堡菜单或侧边栏切换
    const menuButton = page.locator('[data-testid="menu-toggle"]').or(page.locator('[aria-label*="menu"]'))
