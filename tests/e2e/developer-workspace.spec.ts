...
async function navigateToDevWorkspace(page: Page) {
  ...
  await page.waitForSelector("[data-testid='app-container'], .cyberpunk-standalone", { timeout: 10000 });
  // Navigate to dev workspace (click nav item or command palette)
  const devWorkspaceLink = page.locator("text=Developer Workspace, text=开发工作区, text=Dev Workspace").first();
...
test.describe("Panel Navigation", () => {
  ...
  test("should render activity bar with 6 panel icons", async ({ page }) => {
    const icons = page.locator("button").filter({ hasText: /Explorer|Tasks|AI|Search|Quick|Git|文件|任务|搜索|快捷/ });
...
test.describe("File Explorer", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const explorerBtn = page.locator("button").filter({ hasText: /Explorer|文件/ }).first();
  ...
  test("should display file tree with root items", async ({ page }) => {
    ...
    const treeItems = page.locator("[class*='cursor-pointer']").filter({ hasText: /src|components|package/ });
  ...
  test("should create new file via context menu", async ({ page }) => {
    ...
    const newFileBtn = page.locator("button").filter({ hasText: /New File|新建/ }).first();
...
test.describe("AI Assistant", () => {
  ...
  test("should send a message and receive mock response", async ({ page }) => {
    const input = page.locator("input[placeholder*='Ask'], textarea[placeholder*='Ask'], input[placeholder*='输入']").first();
    if (await input.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const response = page.locator("[class*='assistant'], text=/建议|recommend|Hook|pattern/i").first();
  ...
  test("should toggle AI configuration panel", async ({ page }) => {
    const configBtn = page.locator("[title*='config'], [title*='Config'], button:has(svg)").filter({ hasText: /⚙️|Config/ }).first();
...
test.describe("Full Workflow: File → Edit → AI → Git", () => {
  test("complete developer workflow", async ({ page }) => {
    ...
    const aiInput = page.locator("input[placeholder*='Ask'], textarea[placeholder*='Ask'], input[placeholder*='输入']").first();
