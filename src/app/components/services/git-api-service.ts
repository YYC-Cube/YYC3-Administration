...
const MOCK_COMMITS: GitCommitInfo[] = [
  { sha: "a1b2c3d", message: "feat: implement left-panel-page v2.0 with AI Provider + CRUD + Resizable", author: { name: "YYC3 Dev", email: "admin@0379.email", date: new Date(Date.now() - 600000).toISOString() }, filesChanged: 5, url: "#" },
  { sha: "e4f5g6h", message: "feat: task-board v2 with Zustand + DnD + AI inference engine", author: { name: "YYC3 Dev", email: "admin@0379.email", date: new Date(Date.now() - 3600000).toISOString() }, filesChanged: 3, url: "#" },
  { sha: "i7j8k9l", message: "fix: resolve sidebar overlay on iOS Safari viewport", author: { name: "YYC3 Dev", email: "admin@0379.email", date: new Date(Date.now() - 7200000).toISOString() }, filesChanged: 2, url: "#" },
  { sha: "m0n1o2p", message: "refactor: upgrade Tailwind CSS v4 configuration and tokens", author: { name: "YYC3 Dev", email: "admin@0379.email", date: new Date(Date.now() - 86400000).toISOString() }, filesChanged: 8, url: "#" },
  { sha: "q3r4s5t", message: "feat: add command palette with fuzzy search + i18n", author: { name: "YYC3 Dev", email: "admin@0379.email", date: new Date(Date.now() - 172800000).toISOString() }, filesChanged: 4, url: "#" },
];
...
class GitAPIService {
  ...
  async listCommits(perPage: number = 20): Promise<GitOperationResult<GitCommitInfo[]>> {
    ...
    try {
      const res = await this.fetch(`/repos/${this.config!.owner}/${this.config!.repo}/commits?sha=${this.config!.branch}&per_page=${perPage}`);
  ...
  async createCommit(path: string, content: string, message: string, sha?: string): Promise<GitOperationResult<GitCommitInfo>> {
    if (!this.isConfigured) {
    ...
    try {
      ...
      const res = await this.fetch(`/repos/${this.config!.owner}/${this.config!.repo}/contents/${path}`, {
        method: "PUT",
      ...
      return {
        ...
        data: {
          ...
          author: { name: data.commit.author.name, email: data.commit.author.email, date: data.commit.author.date },
          filesChanged: 1,
  ...
  async createBranch(name: string, fromSha: string): Promise<GitOperationResult<GitBranchInfo>> {
    ...
    try {
      ...
      const data = await res.json();
      return { success: true, data: { name, sha: data.object.sha.substring(0, 7), protected: false, current: false } };
    } catch (err: any) {
      return { success: false, error: err.message };
    }
  }
  ...
  async getFileContent(path: string): Promise<GitOperationResult<GitFileContent>> {
    ...
    try {
      const res = await this.fetch(`/repos/${this.config!.owner}/${this.config!.repo}/contents/${path}?ref=${this.config!.branch}`);
      const data = await res.json();
      const content = decodeURIComponent(escape(atob(data.content.replace(/\n/g, ""))));
      return { success: true, data: { path: data.path, content, sha: data.sha, size: data.size, encoding: data.encoding } };
    } catch (err: any) {
      return { success: false, error: err.message };
    }
  }
  ...
  async deleteFile(path: string, message: string, sha: string): Promise<GitOperationResult<{ deleted: boolean }>> {
    if (!this.isConfigured) {
  ...
  /** List pull requests */
  async listPullRequests(state: "open" | "closed" | "all" = "open"): Promise<GitOperationResult<GitPullRequestInfo[]>> {
    if (!this.isConfigured) {
      return {
        success: true,
        data: [
          { number: 42, title: "feat: left-panel v2.0 with AI Provider", state: "open", author: "YYC3 Dev", createdAt: new Date(Date.now() - 3600000).toISOString(), url: "#" },
          { number: 41, title: "feat: task-board Zustand + DnD upgrade", state: "merged" as any, author: "YYC3 Dev", createdAt: new Date(Date.now() - 86400000).toISOString(), url: "#" },
        ],
      };
    }

    try {
      const res = await this.fetch(`/repos/${this.config!.owner}/${this.config!.repo}/pulls?state=${state}`);
      const data = await res.json();
      return {
        success: true,
