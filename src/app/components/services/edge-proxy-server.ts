...
function validateRequest(body: any): { valid: true; request: ProxyRequest } | { valid: false; error: string } {
...
export async function handler(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (!apiKey) {
      return new Response(JSON.stringify({ error: `API key not configured for provider: ${proxyReq.provider}` }), {
    ...
    console.log(`[YYC³ AI Proxy] ${proxyReq.provider}/${proxyReq.model} - ${result.usage?.totalTokens ?? "?"} tokens - IP: ${clientIP.substring(0, 8)}***`);
