...
export function createMockGitAPIService() {
  const commits = [createMockCommit(), createMockCommit({ sha: "e4f5g6h", message: "fix: resolve viewport issue" })];
  const branches = [createMockBranch({ name: "main", protected: true }), createMockBranch({ name: "dev", current: true })];
  ...
  return {
    ...
    listPullRequests: async () => ({
      success: true as const,
      data: [{ number: 42, title: "feat: test PR", state: "open" as const, author: "YYC3", createdAt: new Date().toISOString(), url: "#" }],
    }),
  };
}
