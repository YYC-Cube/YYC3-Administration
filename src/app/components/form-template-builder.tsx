import {
  AlignLeft, Calendar, Check, CheckCircle2, ClipboardList, Copy,
  Download, Eye, GripVertical, Hash, List, Plus, Save,
  Settings2, Sliders, Star,
  ToggleLeft, Trash2, Type, Upload, X,
} from "lucide-react";
...
function saveTpl(templates: CustomTemplate[]) {
  try { localStorage.setItem(CUSTOM_TEMPLATES_KEY, JSON.stringify(templates)); } catch { /* */ }
}
...
export function FormTemplateBuilder() {
  ...
  const addField = useCallback((type: FieldType) => {
    ...
    const newField: BuilderField = {
      ...
      placeholder: type === "text" || type === "textarea" || type === "number" ? "请输入…" : undefined,
      required: false,
      options: (type === "select" || type === "radio" || type === "checkbox") ? ["选项 1", "选项 2", "选项 3"] : undefined,
      min: type === "slider" ? 0 : type === "number" ? 0 : undefined,
      max: type === "slider" ? 100 : type === "number" ? 999999 : undefined,
      step: type === "slider" ? 1 : undefined,
      defaultValue: type === "toggle" ? false : type === "rating" ? 0 : type === "slider" ? 50 : undefined,
      color: info.color,
    };
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (data && data.fields && Array.isArray(data.fields)) {
          ...
          setFields(data.fields.map((f: Record<string, unknown>) => ({ ...f } as unknown as FieldDef)));
          setEditingFieldId(null);
    ...
    <div style={{ animation: "spring-in 0.4s var(--spring-easing) both" }}>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-5">
        ...
        <div className="flex items-center gap-2">
          {/* Color picker */}
          <div className="flex items-center gap-1 px-2 py-1.5 rounded-xl" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
            {COLOR_OPTIONS.map(c => (
              <button
                key={c}
      ...
      {showSaved && savedTemplates.length > 0 && (
        <NeonCard color="#00d4ff" hoverable={false} noReveal>
          <div className="space-y-2 mb-4" style={{ animation: "spring-in 0.25s var(--spring-easing) both" }}>
            <h4 className="text-[10px] text-white/25 uppercase tracking-wider mb-2">已保存模板</h4>
            {savedTemplates.map(tpl => (
              <div key={tpl.id} className="flex items-center gap-3 px-3 py-2 rounded-xl" style={{ background: "rgba(10,10,10,0.5)", border: "1px solid rgba(255,255,255,0.04)" }}>
                <div className="w-6 h-6 rounded-lg flex items-center justify-center" style={{ background: `${tpl.color}15` }}>
                  <ClipboardList className="w-3 h-3" style={{ color: tpl.color }} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] text-white/50 truncate">{tpl.title}</p>
                  <p className="text-[8px] text-white/15">{tpl.fields.length} 字段 · {new Date(tpl.createdAt).toLocaleDateString("zh-CN")}</p>
                </div>
                <button onClick={() => loadTemplate(tpl)} className="text-[9px] px-2 py-1 rounded-lg" style={{ background: `${tpl.color}10`, color: tpl.color, border: `1px solid ${tpl.color}20` }}>
                  加载
                </button>
                <button onClick={() => deleteSaved(tpl.id)} className="p-1 text-white/15 hover:text-[#005f73] transition-colors">
                  <X className="w-3 h-3" />
                </button>
              </div>
            ))}
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <button
          ...
          style={{ background: "rgba(0,240,255,0.04)", border: "1px solid rgba(0,240,255,0.15)", color: "#00f0ff" }}
        >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div className={showPreview ? "xl:col-span-2" : "xl:col-span-3"}>
          ...
          <NeonCard color={tplColor} hoverable={false} noReveal>
            <h4 className="text-[10px] text-white/25 uppercase tracking-wider mb-3">字段面板 · 点击添加</h4>
            ...
            <div className="border-t pt-4" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
              ...
              {fields.length === 0 && (
                <div className="text-center py-10 border-2 border-dashed rounded-2xl" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
                ...
                {fields.map((field, idx) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background: isEditing ? `${field.color || tplColor}08` : "rgba(10,10,10,0.3)",
                        borderColor: isDragOver ? `${tplColor}60` : isEditing ? `${field.color || tplColor}30` : "rgba(255,255,255,0.04)",
                        boxShadow: isDragOver ? `0 0 12px ${tplColor}20` : isEditing ? `0 0 8px ${field.color || tplColor}10` : "none",
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{ background: `${field.color || tplColor}10`, border: `1px solid ${field.color || tplColor}20` }}
                      >
                        <FieldIcon className="w-3.5 h-3.5" style={{ color: field.color || tplColor }} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs text-white/50 truncate">{field.label}</p>
                        <p className="text-[9px] text-white/15">{fInfo?.label || field.type}{field.required ? " · 必填" : ""}</p>
                      </div>
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => setEditingFieldId(isEditing ? null : field.id)} className="p-1 rounded text-white/20 hover:text-white/50 transition-colors">
                          <Settings2 className="w-3 h-3" />
                        </button>
                        <button onClick={() => duplicateField(field.id)} className="p-1 rounded text-white/20 hover:text-white/50 transition-colors">
                          <Copy className="w-3 h-3" />
                        </button>
                        <button onClick={() => removeField(field.id)} className="p-1 rounded text-white/20 hover:text-[#005f73] transition-colors">
          ...
          {editingField && (
            <div className="mt-4" style={{ animation: "spring-in 0.25s var(--spring-easing) both" }}>
              <NeonCard color={editingField.color || tplColor} hoverable={false} noReveal>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button onClick={() => setEditingFieldId(null)} className="p-1 text-white/15 hover:text-white/40 transition-colors">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      style={{ background: "rgba(10,10,10,0.5)", border: "1px solid rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.7)", outline: "none" }}
                    />
                  </div>
                  {/* Placeholder */}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      style={{
                        background: editingField.required ? "rgba(0,95,115,0.25)" : "rgba(255,255,255,0.06)",
                        border: `1px solid ${editingField.required ? "rgba(0,95,115,0.5)" : "rgba(255,255,255,0.1)"}`,
                      }}
                    >
                      <div className="absolute top-0.5 w-4.5 h-4.5 rounded-full transition-all" style={{ left: editingField.required ? 18 : 3, width: 18, height: 18, background: editingField.required ? "#005f73" : "rgba(255,255,255,0.3)" }} />
                    </button>
                  </div>
                  {/* Color */}
                  <div>
                    ...
                    <div className="flex gap-1.5">
                      {COLOR_OPTIONS.map(c => (
                        <button
                          ...
                          style={{ background: c, border: editingField.color === c ? "2px solid white" : "2px solid transparent", transform: editingField.color === c ? "scale(1.2)" : "scale(1)" }}
                        />
                      ))}
                    </div>
                  </div>
                  ...
                  {(editingField.type === "select" || editingField.type === "radio" || editingField.type === "checkbox") && (
                    <div className="sm:col-span-2">
                      <label className="text-[9px] text-white/20 mb-1 block">选项（每行一个）</label>
                      <textarea
                        ...
                        style={{ background: "rgba(10,10,10,0.5)", border: "1px solid rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.7)", outline: "none", scrollbarWidth: "none" }}
                      />
                    </div>
                  )}
                  {/* Slider range */}
                  ...
                  {editingField.type === "text" && (
                    <div>
                      ...
                      <select
                        ...
                        onChange={e => updateField(editingField.id, { validation: e.target.value as any })}
                        className="w-full px-3 py-2 text-xs rounded-lg appearance-none"
                        style={{ background: "rgba(10,10,10,0.5)", border: "1px solid rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.6)", outline: "none" }}
                      >
                        <option value="none" style={{ background: "#0a0a0a" }}>无</option>
                        <option value="email" style={{ background: "#0a0a0a" }}>邮箱</option>
                        <option value="phone" style={{ background: "#0a0a0a" }}>手机号</option>
                        <option value="url" style={{ background: "#0a0a0a" }}>URL</option>
                      </select>
                    </div>
                  )}
                </div>
          ...
          <div className="xl:col-span-1" style={{ animation: "spring-in 0.3s var(--spring-easing) both" }}>
            <NeonCard color={tplColor} hoverable={false} noReveal>
              ...
              <div className="space-y-3">
                <div className="px-3 py-2 rounded-xl" style={{ background: `${tplColor}08`, border: `1px solid ${tplColor}15` }}>
                  <p className="text-xs" style={{ color: tplColor }}>{title || "未命名模板"}</p>
                  <p className="text-[9px] text-white/20">{subtitle}</p>
                </div>
                {fields.map((f, i) => {
                  const FIcon = FIELD_PALETTE.find(p => p.type === f.type)?.icon || Type;
                  return (
                    <div key={f.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg" style={{ background: "rgba(255,255,255,0.02)" }}>
                      <FIcon className="w-3 h-3 shrink-0" style={{ color: `${f.color || tplColor}60` }} />
              ...
              <div className="mt-3 pt-2 border-t flex justify-between text-[9px]" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
                <span className="text-white/15">{fields.length} 个字段</span>
                <span className="text-white/15">{fields.filter(f => f.required).length} 必填</span>
              </div>
            </NeonCard>
          </div>
        )}
