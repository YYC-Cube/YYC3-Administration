...
export function ChatInterface({ compact = false, onInsertReady }: ChatInterfaceProps) {
  ...
  useEffect(() => {
    if (onInsertReady) onInsertReady((text: string) => { insertToEditorRef.current = (t: string) => {} })
  }, [onInsertReady])
    ...
    async (userContent: string): Promise<string> => {
      ...
      try {
        ...
        if (provider === 'ollama') {
          ...
          const url = base.includes('/api/chat') ? base : base.replace(/\/api\/.*$/, '') + '/api/chat'
          resp = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: modelName, messages: [{ role: 'user', content: userContent }], stream: false }) })
        } else if (endpoint.includes('anthropic.com')) {
          resp = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01', 'anthropic-dangerous-direct-browser-access': 'true' }, body: JSON.stringify({ model: modelName, max_tokens: 2048, messages: [{ role: 'user', content: userContent }] }) })
        } else {
          resp = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + apiKey }, body: JSON.stringify({ model: modelName, messages: [{ role: 'user', content: userContent }], stream: false, max_tokens: 2048, temperature: 0.7 }) })
        }
        if (!resp.ok) {
          const errText = await resp.text().catch(() => '')
          let detail = ''
          try { const j = JSON.parse(errText); detail = j.error?.message || j.message || errText.slice(0, 300) }
          catch { detail = errText.slice(0, 300) }
          return t('chat.apiError', { msg: 'HTTP ' + resp.status + ' - ' + detail })
        }
        const data = await resp.json().catch(() => null)
        if (provider === 'ollama') return data?.message?.content || data?.response || t('chat.fallbackReply')
        if (endpoint.includes('anthropic.com')) return data?.content?.[0]?.text || t('chat.fallbackReply')
        return data?.choices?.[0]?.message?.content || data?.result || t('chat.fallbackReply')
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : 'Unknown error'
        if (msg.includes('Failed to fetch') || msg.includes('NetworkError')) return t('chat.apiError', { msg: t('chat.networkError') })
  ...
  const handleSend = useCallback(async () => {
    ...
    const userMsg: ChatMessage = {
      id: 'm_' + Date.now(), role: 'user', content: userText,
      timestamp: new Date(), quoteContent: quoteContent ?? undefined,
    }
    addMessage(userMsg)
  ...
  }, [input, quoteContent, streamState.isStreaming, callLLM, startSimulatedStream, addMessage, updateMessage, addActivity, t])
  ...
  const handleCopy = useCallback(async (text: string, msgId?: string) => {
    ...
    if (msgId) { setCopiedMsgId(msgId); setTimeout(() => setCopiedMsgId(null), 2000) }
  }, [])
  ...
  const handleExport = useCallback((format: 'md' | 'json') => {
    ...
    if (format === 'md') {
      ...
      content = JSON.stringify(messages.map(({ id, role, content, timestamp }) => ({ id, role, content, timestamp })), null, 2)
  ...
  const renderMessage = (msg: ChatMessage, i: number) => {
    ...
    return (
      <div key={msg.id} className={'flex gap-3 ' + (msg.role === 'user' ? 'flex-row-reverse' : '')}
        style={{ animation: 'bubble-in 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275) ' + (i * 0.03) + 's both' }}>
        <div className="shrink-0 w-8 h-8 rounded-lg flex items-center justify-center border"
          ...
          {msg.role === 'ai' ? <Bot className="w-4 h-4" style={{ color: tc.primary }} /> : <User className="w-4 h-4" style={{ color: tc.secondary }} />}
        </div>

        <div className={'relative max-w-[75%] rounded-2xl px-4 py-3 ' + (compact ? 'max-w-[85%]' : '')}
          style={{
            ...
            boxShadow: msg.role === 'ai'
              ? (msg.isError ? '0 0 15px ' + tc.alpha(tc.muted, 0.1) : '0 0 15px ' + tc.alpha(tc.primary, 0.1))
              : '0 0 15px ' + tc.alpha(tc.secondary, 0.1),
          }}>
          {msg.quoteContent && (
            <div className="mb-2 pl-3 border-l-2 text-[11px] text-white/30 italic" style={{ borderColor: tc.alpha(tc.primary, 0.3) }}>
              {msg.quoteContent.slice(0, 100)}{msg.quoteContent.length > 100 ? '...' : ''}
            </div>
          )}
          ...
          {msg.role === 'ai' ? (
            <div className="text-sm leading-relaxed prose prose-invert max-w-none"
              ...
              {displayContent ? (
                <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeHighlight]}
                  components={{
                    code({ className, children, ...props }: any) {
                      ...
                      if (isInline) {
                        return <code className="px-1 py-0.5 rounded text-xs" style={{ background: tc.alpha(tc.primary, 0.1), color: tc.primary }} {...props}>{children}</code>
                      }
                      return (
                        <div className="relative group my-3">
                          <div className="flex items-center justify-between px-3 py-1.5 rounded-t-lg text-[10px]"
                            style={{ background: 'rgba(0,0,0,0.4)', color: tc.textMuted }}>
                            <span>{className ? className.replace('language-', '') : 'code'}</span>
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => handleCopy(codeStr)} className="px-2 py-0.5 rounded hover:bg-white/10 transition-colors" title="Copy">
                                <Copy className="w-3 h-3" />
                              </button>
                              <button onClick={() => handleInsertCode(codeStr)}
                          ...
                          <pre className="m-0 rounded-b-lg" style={{ background: 'rgba(0,0,0,0.3)' }}>
                ...
                isStreaming && <span className="inline-block w-2 h-4 ml-0.5 animate-pulse rounded-sm" style={{ background: tc.primary }} />
              )}
            </div>
          ) : (
            <p className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: tc.textPrimary }}>{displayContent}</p>
          )}

          <div className="flex items-center justify-between mt-2 pt-2 border-t" style={{ borderColor: tc.borderSubtle }}>
            ...
            <div className="flex gap-1">
              {msg.role === 'ai' && displayContent && !isStreaming && (
                <>
                  <button onClick={() => handleCopy(displayContent, msg.id)}
                    className="p-1 rounded hover:bg-white/10 transition-colors" title={t('chat.copy')}>
                    {copiedMsgId === msg.id ? <Check className="w-3 h-3 text-green-400" /> : <Copy className="w-3 h-3 text-white/30 hover:text-white/60" />}
                  </button>
                  <button onClick={() => handleQuote(displayContent)}
                    className="p-1 rounded hover:bg-white/10 transition-colors" title="Quote reply">
                    <MessageSquare className="w-3 h-3 text-white/30 hover:text-white/60" />
                  </button>
                  <button onClick={() => handleRetry(msg.id)}
                    className="p-1 rounded hover:bg-white/10 transition-colors" title={t('chat.retry')}>
  ...
  return (
    <div className="flex h-full" style={{ background: tc.bgBase }}>
      {/* Session Sidebar */}
      <div className="w-56 shrink-0 flex flex-col border-r" style={{ borderColor: tc.alpha(tc.primary, 0.1), background: tc.alpha(tc.bgCard, 0.5) }}>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {sessions.map((s) => (
            <div key={s.id}
              className={'group flex items-center justify-between px-3 py-2 rounded-xl text-xs cursor-pointer transition-all ' + (s.id === activeId ? 'border' : 'hover:bg-white/5')}
              style={{
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <div className="shrink-0 flex items-center justify-between px-4 py-2 border-b" style={{ borderColor: tc.alpha(tc.primary, 0.1) }}>
          <div className="flex items-center gap-2">
            <div className={'w-1.5 h-1.5 rounded-full ' + (activeModel ? 'bg-emerald-400' : 'bg-white/15')}
              style={activeModel ? { boxShadow: '0 0 4px ' + tc.success } : {}} />
            <span className="text-[10px] text-white/30">
              {activeModel ? activeModel.name + ' - ' + activeModel.provider : t('chat.modelNotConfigured')}
            </span>
            {streamState.isStreaming && (
              <span className="text-[9px] px-1.5 py-0.5 rounded-full animate-pulse" style={{ background: tc.alpha(tc.primary, 0.15), color: tc.primary }}>
                Generating...
              </span>
            )}
          </div>
          <button onClick={openModelSettings} className="p-1 rounded-lg hover:bg-white/5 transition-colors group" title={t('header.aiModel')}>
            <Settings2 className="w-3.5 h-3.5 text-white/20 group-hover:text-white/60 transition-colors" />
          </button>
        </div>

        <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin" style={{ scrollbarWidth: 'none' }}>
          {messages.length === 0 && !streamState.isStreaming && (
            <div className="flex flex-col items-center justify-center h-full py-16">
              <Bot className="w-12 h-12 mb-4" style={{ color: tc.alpha(tc.primary, 0.3) }} />
              <p className="text-sm mb-6" style={{ color: tc.textMuted }}>How can I help you?</p>
          ...
          {streamState.isStreaming && !messages.find((m) => isStreamingFor(m.id)) && (
            <div className="flex gap-3" style={{ animation: 'bubble-in 0.4s var(--spring-easing) both' }}>
              <div className="shrink-0 w-8 h-8 rounded-lg flex items-center justify-center border"
                style={{ background: tc.alpha(tc.primary, 0.1), borderColor: tc.alpha(tc.primary, 0.4) }}>
                <Bot className="w-4 h-4" style={{ color: tc.primary }} />
              </div>
              <div className="rounded-2xl px-5 py-4 border" style={{ background: tc.alpha(tc.bgBase, 0.8), borderColor: tc.alpha(tc.primary, 0.2) }}>
                <div className="flex gap-1.5 items-center">
                  {[0, 1, 2].map((i) => (
                    <div key={i} className="w-2 h-2 rounded-full"
                      style={{ background: tc.primary, animation: 'thinking-pulse 1.4s ease-in-out ' + (i * 0.2) + 's infinite' }} />
                  ))}
                  <span className="ml-2 text-xs" style={{ color: tc.alpha(tc.primary, 0.6) }}>{t('chat.aiThinking')}</span>
                </div>
              </div>
            </div>
          )}
        ...
        {quoteContent && (
          <div className="mx-3 px-3 py-2 rounded-xl flex items-center gap-2 text-xs border"
            style={{ background: tc.alpha(tc.primary, 0.05), borderColor: tc.alpha(tc.primary, 0.2), color: tc.textMuted }}>
            <MessageSquare className="w-3 h-3 shrink-0" style={{ color: tc.primary }} />
            <span className="truncate flex-1">{quoteContent.slice(0, 80)}{quoteContent.length > 80 ? '...' : ''}</span>
            <button onClick={() => setQuoteContent(null)} className="p-0.5 rounded hover:bg-white/10 transition-colors">
              <X className="w-3 h-3" />
            </button>
          </div>
        )}
        ...
        <div className="p-3 border-t" style={{ borderColor: tc.alpha(tc.primary, 0.1) }}>
          <div className="relative flex items-end gap-2 rounded-2xl border px-4 py-3 transition-all duration-300"
            style={{ background: tc.alpha(tc.bgBase, 0.6), borderColor: tc.alpha(tc.primary, 0.2), backdropFilter: 'blur(10px)' }}>
            ...
            {streamState.isStreaming ? (
              <button onClick={stopStream}
                ...
                style={{ background: tc.alpha(tc.danger, 0.3), boxShadow: '0 0 10px ' + tc.alpha(tc.danger, 0.2) }}>
                <Square className="w-4 h-4 text-white" />
              </button>
            ) : (
              <button onClick={handleSend} disabled={!input.trim()}
                className="shrink-0 p-2 rounded-xl transition-all duration-300 disabled:opacity-30"
                style={{ background: input.trim() ? tc.gradientPrimary : 'rgba(255,255,255,0.05)', boxShadow: input.trim() ? tc.shadowGlow : 'none' }}>
