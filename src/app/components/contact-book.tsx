import {
  AlertTriangle, ArrowDownRight, ArrowUpRight, BarChart3, BookOpen, Brain, Building2,
  Check, ChevronDown, ChevronRight, Clock, Copy,
  Crown, Download, Edit3, Eye, Filter,
  Handshake, HeartHandshake, Mail, MapPin, Megaphone,
  MessageCircle, MoreHorizontal, Phone, PhoneCall, Plus, RefreshCw, Search,
  Shield, Sparkles, Star, StarOff, Tag,
  Target, Trash2, TrendingUp, Upload,
...
const STAGE_META_KEYS: Record<string, { icon: LucideIcon; colorKey: keyof ReturnType<typeof useThemeColors>; sublabel: string }> = {
...
// ---- AI Score Badge ----
const AIScoreBadge = memo(function AIScoreBadge({ score, t }: { score: number; t: (k: string) => string }) {
  const color = score >= 90 ? "#00ffc8" : score >= 70 ? "#00ffcc" : score >= 50 ? "#008b9d" : "#005f73";
  const label = score >= 90 ? t("cb.excellent") : score >= 70 ? t("cb.good") : score >= 50 ? t("cb.average") : t("cb.needImprovement");
  return (
    <div className="flex items-center gap-1.5">
      <div className="relative w-8 h-8">
        ...
        <span className="absolute inset-0 flex items-center justify-center text-[8px] tabular-nums" style={{ color }}>
          {score}
        </span>
      </div>
...
function RiskBadge({ level, t }: { level: string; t: (k: string) => string }) {
  ...
  return (
    <span className="text-[9px] px-1.5 py-0.5 rounded-full" style={{ background: `${m.color}15`, color: m.color, border: `1px solid ${m.color}30` }}>
      {t(m.labelKey)}
    </span>
  );
}
...
function ContactFormModal({
  ...
  const [form, setForm] = useState<SharedContact>(
    contact || {
      id: `c${Date.now()}`, name: "", phone: "", email: "", company: "", position: "",
      stage: "获客", tags: [], aiScore: 50, aiInsights: ["新建联系人，AI 将自动分析"], starred: false,
      address: "", source: "手动录入", createdAt: new Date().toISOString().slice(0, 10),
      lastContact: "刚刚", totalCalls: 0, totalValue: 0, notes: "", riskLevel: "medium",
    }
  );

  const updateField = (key: keyof SharedContact, value: string | number | string[]) => setForm(prev => ({ ...prev, [key]: value }));
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center" style={{ animation: "fade-in 0.2s ease-out both" }}>
      <div className="absolute inset-0 bg-black/70" style={{ backdropFilter: "blur(4px)" }} onClick={onClose} />
      <div
        className="relative w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl border p-6"
        style={{
          background: "rgba(10,10,10,0.96)", borderColor: "rgba(0,240,255,0.3)",
          boxShadow: "0 0 40px rgba(0,240,255,0.15)", scrollbarWidth: "none",
          animation: "spring-in 0.4s var(--spring-easing) both",
        }}
      >
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-sm tracking-wider" style={{ color: "#00f0ff", textShadow: "0 0 10px rgba(0,240,255,0.5)" }}>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                value={form.source} onChange={e => updateField("source", e.target.value)}
                className="w-full px-3 py-2 rounded-xl text-sm text-white/80 outline-none transition-all"
                style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(0,240,255,0.15)" }}
              >
                {["手动录入", "官网注册", "行业展会", "客户推荐", "搜索引擎", "社交媒体", "合作伙伴", "行业会议", "线下活动"].map(s => (
        ...
        {/* Actions */}
        <div className="flex justify-end gap-3 mt-6 pt-4" style={{ borderTop: "1px solid rgba(255,255,255,0.05)" }}>
          <button
            onClick={onClose}
...
function ContactDetailPanel({
  ...
  return (
    <div
      ...
      style={{
        background: "rgba(5,5,5,0.95)", borderColor: "rgba(0,240,255,0.15)",
    ...
    >
      ...
      <div className="p-5 border-b" style={{ borderColor: "rgba(0,240,255,0.08)" }}>
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex gap-1.5">
            <button onClick={onEdit} className="p-1.5 rounded-lg hover:bg-white/5 transition-colors" title={t("common.edit")}>
              <Edit3 className="w-3.5 h-3.5 text-white/30" />
            </button>
            <button onClick={onCall} className="p-1.5 rounded-lg hover:bg-white/5 transition-colors" title={t("cb.phone")}>
        ...
        <div className="text-center">
          ...
          <p className="text-[11px] text-white/30">{contact.position} · {contact.company}</p>
            ...
            <div className="h-2 rounded-full bg-white/5 overflow-hidden">
              <div
                ...
                style={{
                  ...
                  background: contact.aiScore >= 90 ? "linear-gradient(90deg, #00ffc8, #00f0ff)" :
                    contact.aiScore >= 70 ? "linear-gradient(90deg, #00ffcc, #00ffc8)" :
      ...
      {/* Contact Info */}
      <div className="p-5 border-b" style={{ borderColor: "rgba(0,240,255,0.08)" }}>
        <h4 className="text-[10px] text-white/30 uppercase tracking-wider mb-3">{t("cb.contactInfo")}</h4>
      ...
      {/* Stats */}
      <div className="p-5 border-b" style={{ borderColor: "rgba(0,240,255,0.08)" }}>
        <h4 className="text-[10px] text-white/30 uppercase tracking-wider mb-3">{t("cb.interactionData")}</h4>
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: t("cb.callCount"), value: contact.totalCalls.toString(), color: "#00ffcc" },
            { label: t("cb.customerValue"), value: `¥${(contact.totalValue / 1000).toFixed(0)}K`, color: "#00ffc8" },
            { label: t("cb.lastContact"), value: contact.lastContact, color: "#00f0ff" },
            { label: t("cb.createdDate"), value: contact.createdAt, color: "#00d4ff" },
          ].map((s, i) => (
            <div key={i} className="rounded-xl p-2.5" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.04)" }}>
              <p className="text-[9px] text-white/20 mb-0.5">{s.label}</p>
              <p className="text-xs tabular-nums" style={{ color: s.color }}>{s.value}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="p-5 border-b" style={{ borderColor: "rgba(0,240,255,0.08)" }}>
        <h4 className="text-[10px] text-white/30 uppercase tracking-wider mb-3">{t("cb.lifecycleAdvance")}</h4>
          ...
          {(["获客", "转化", "成交", "服务", "忠诚"] as const).map((stage, i) => {
            ...
            return (
              <button
                ...
                style={{
                  background: isActive ? `${meta.color}20` : isPast ? `${meta.color}08` : "rgba(255,255,255,0.02)",
                  border: `1px solid ${isActive ? `${meta.color}50` : isPast ? `${meta.color}20` : "rgba(255,255,255,0.05)"}`,
                  color: isActive ? meta.color : isPast ? `${meta.color}80` : "rgba(255,255,255,0.2)",
      ...
      <div className="p-5">
        ...
        <p className="text-[11px] text-white/40 leading-relaxed">{contact.notes || t("cb.noNotes")}</p>
      </div>
    </div>
  );
}
...
export function ContactBookPage() {
  ...
  const { t } = useI18n();
  const {
    contacts, addContact, updateContact, deleteContact, toggleStar, updateStage,
  } = useContacts();
  ...
  const filteredContacts = useMemo(() => {
    ...
    result = [...result].sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        default: va = a.aiScore; vb = b.aiScore;
      }
  ...
  const stats = useMemo(() => {
    ...
    const stageCount = Object.fromEntries(
      (["获客", "转化", "成交", "服务", "忠诚"] as const).map(s => [s, contacts.filter(c => c.stage === s).length])
    );
    return { total, starred, avgAI, totalValue, stageCount };
  }, [contacts]);

  // Actions — now using shared context
  const handleSave = useCallback((c: SharedContact) => {
  ...
  const handleAIEnhance = useCallback((id: string) => {
    setAiEnhancing(id);
    setTimeout(() => {
  ...
  const handleStageChange = useCallback((id: string, stage: SharedContact["stage"]) => {
      ...
      reader.onload = (ev) => {
        try {
          ...
          if (Array.isArray(data)) {
            ...
            data.filter((c: Record<string, unknown>) => c.id && c.name && !ids.has(c.id as string)).forEach((c: SharedContact) => addContact(c));
          }
        } catch { /* */ }
      };
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="shrink-0 p-6 pb-0">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="tracking-wider flex items-center gap-3" style={{ color: "#00f0ff", textShadow: "0 0 15px rgba(0,240,255,0.5)" }}>
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-3 mb-4">
            {[
              { label: t("cb.totalContacts"), value: stats.total.toString(), icon: Users, color: "#00f0ff" },
              { label: t("cb.starredContacts"), value: stats.starred.toString(), icon: Star, color: "#00ffcc" },
              { label: t("cb.avgAiScore"), value: stats.avgAI.toString(), icon: Brain, color: "#00d4ff" },
              { label: t("cb.totalValue"), value: `¥${(stats.totalValue / 1000).toFixed(0)}K`, icon: TrendingUp, color: "#00ffc8" },
            ].map((m, i) => {
              const Icon = m.icon;
              return (
                <div key={i} className="rounded-xl p-3 border transition-all duration-300" style={{ background: "rgba(10,10,10,0.5)", borderColor: `${m.color}20` }}>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[9px] text-white/20 uppercase tracking-wider">{m.label}</p>
                      <p className="text-lg tabular-nums mt-0.5" style={{ color: m.color, textShadow: `0 0 8px ${m.color}40` }}>{m.value}</p>
                    </div>
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="flex-1 relative">
              ...
              <input
                ...
                style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(0,240,255,0.12)" }}
            ...
            <button
              ...
              style={{
                background: (filterStage || filterTag) ? "rgba(0,212,255,0.1)" : "rgba(255,255,255,0.04)",
            ...
            >
              ...
              {t("cb.filter")}
              {(filterStage || filterTag) && <span className="w-1.5 h-1.5 rounded-full bg-[#00d4ff]" />}
            </button>
            {/* Sort */}
            <select
            ...
            >
              <option value="aiScore-desc" style={{ background: "#0a0a0a" }}>AI ↓</option>
              <option value="aiScore-asc" style={{ background: "#0a0a0a" }}>AI ↑</option>
              <option value="totalValue-desc" style={{ background: "#0a0a0a" }}>¥ ↓</option>
              <option value="totalValue-asc" style={{ background: "#0a0a0a" }}>¥ ↑</option>
              <option value="name-asc" style={{ background: "#0a0a0a" }}>A-Z</option>
              <option value="name-desc" style={{ background: "#0a0a0a" }}>Z-A</option>
            </select>
          </div>
          ...
          {showFilters && (
            <div
              ...
              style={{ background: "rgba(10,10,10,0.6)", borderColor: "rgba(0,212,255,0.15)", animation: "spring-in 0.3s var(--spring-easing) both" }}
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-[10px] text-white/30 uppercase tracking-wider">{t("cb.filterConditions")}</span>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(["获客", "转化", "成交", "服务", "忠诚"] as const).map(stage => {
                    ...
                    return (
                      ...
                      >
                        {stage}
                        <span className="text-[8px] opacity-60">({stats.stageCount[stage] || 0})</span>
                      </button>
                    );
                  })}
                </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6" style={{ scrollbarWidth: "none" }}>
          <div className="text-[10px] text-white/15 mb-2">{t("cb.contactCount", { count: filteredContacts.length })}</div>
          <div className="space-y-1.5">
            {filteredContacts.map((contact, i) => {
              ...
              return (
                ...
                >
                  {/* Star */}
                  <button
                    onClick={e => { e.stopPropagation(); handleToggleStar(contact.id); }}
                    className="shrink-0 p-0.5 transition-colors"
                  >
                    {contact.starred
                      ? <Star className="w-4 h-4 text-[#00ffcc] fill-[#00ffcc]" style={{ filter: "drop-shadow(0 0 4px rgba(0,255,204,0.4))" }} />
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {contact.tags.slice(0, 2).map(tag => (
                        <span
                          ...
                          style={{ background: `${TAG_COLORS[tag] || "#00f0ff"}12`, color: `${TAG_COLORS[tag] || "#00f0ff"}90` }}
                        >
                          {tag}
                        </span>
                      ))}
                    </div>
                    <p className="text-[10px] text-white/25 truncate">{contact.position} · {contact.company}</p>
                  </div>
                  ...
                  <div className="hidden sm:block">
                    <span
                      ...
                      style={{ background: `${stageMeta.color}15`, color: stageMeta.color, border: `1px solid ${stageMeta.color}25` }}
                    >
                      {contact.stage}
                    </span>
                  </div>
                  ...
                  <div className="hidden lg:block text-right min-w-[70px]">
                    <p className="text-xs text-[#00ffc8] tabular-nums" style={{ textShadow: "0 0 6px rgba(0,255,200,0.3)" }}>
                  ...
                  {/* Last Contact */}
                  <span className="text-[10px] text-white/15 hidden xl:block min-w-[60px] text-right">{contact.lastContact}</span>

                  {/* Actions */}
                  <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={e => { e.stopPropagation(); handleAIEnhance(contact.id); }}
                    ...
                    <button
                      onClick={e => { e.stopPropagation(); setEditContact(contact); setShowAddModal(true); }}
      ...
      {selectedContact && (
        <div className="hidden xl:block w-80 shrink-0">
          <ContactDetailPanel
            ...
            onEdit={() => { setEditContact(selectedContact); setShowAddModal(true); }}
