...
export const PWAInstallPrompt = memo(function PWAInstallPrompt() {
  ...
  const handleDismiss = useCallback(() => {
    setShowBanner(false);
    try { localStorage.setItem(PWA_DISMISSED_KEY, String(Date.now())); } catch { /* ignore */ }
  }, []);
  ...
  const offlineIndicator = !isOnline && (
    ...
    >
      <WifiOff className="w-4 h-4 text-[#005f73]" style={{ animation: "neon-pulse 2s ease-in-out infinite" }} />
  ...
  return (
    <>
      ...
      >
        {/* Accent top bar */}
        <div className="h-0.5" style={{ background: "linear-gradient(90deg, #00f0ff, #00d4ff, #00ffcc)" }} />
        ...
        <div className="p-4">
          <div className="flex items-start gap-3 mb-3">
            ...
            <button onClick={handleDismiss} className="p-1 rounded-lg hover:bg-white/5 transition-colors shrink-0">
