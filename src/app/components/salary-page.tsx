import {
  ArrowDownRight, ArrowUpRight, BarChart3, Brain,
  Calculator, Calendar, CheckCircle2, Clock,
  Download, FileText, Percent, Plus, ShieldCheck,
  ...
  return (
    <div className="h-full overflow-y-auto p-6" style={{ scrollbarWidth: 'none', animation: 'spring-in 0.4s var(--spring-easing) both' }}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="tracking-wider flex items-center gap-3" style={{ color: '#8b5cf6', textShadow: '0 0 15px rgba(139,92,246,0.5)' }}>
            <Wallet className="w-6 h-6" />
            {t('salary.title')}
          </h2>
          <p className="text-xs text-white/25 mt-1 tracking-wider">Salary Management System — Intelligent Payroll Engine</p>
        </div>
...
function SalaryDashboard() {
  ...
  const kpiCards = useMemo(() => [
    { label: t('salary.kpi.grossPay'), value: '¥1,280,000', change: '+5.2%', up: true, icon: TrendingUp, color: '#10b981' },
    { label: t('salary.kpi.netPay'), value: '¥968,000', change: '+4.8%', up: true, icon: Wallet, color: '#3b82f6' },
    { label: t('salary.kpi.companyCost'), value: '¥358,400', change: '+3.1%', up: true, icon: Users, color: '#8b5cf6' },
    { label: t('salary.kpi.avgSalary'), value: '¥12,800', change: '+2.1%', up: true, icon: BarChart3, color: '#06b6d4' },
  ], [t])
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        ...
        <NeonCard color="#8b5cf6" hoverable={false}>
          <h3 className="text-xs text-white/40 mb-4 uppercase tracking-wider">{t('salary.deptBreakdown')}</h3>
            ...
            {deptBreakdown.map((d, i) => (
              <div key={i}>
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className="text-[10px]" style={{ color: d.color }}>{d.total} · {d.pct}%</span>
                </div>
                <div className="w-full h-1.5 rounded-full bg-white/5">
                  <div className="h-full rounded-full transition-all duration-500" style={{ width: `${d.pct}%`, background: d.color, boxShadow: `0 0 6px ${d.color}40` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <NeonCard color="#10b981" hoverable={false}>
          <h3 className="text-xs text-white/40 mb-4 uppercase tracking-wider">{t('salary.composition')}</h3>
          {[
      ...
      <NeonCard color="#8b5cf6" hoverable={false}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-xs text-white/40 uppercase tracking-wider">AI {t('salary.aiInsight')}</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {[
            { title: t('salary.ai.costForecast'), desc: '预计下半年人力成本增长 6-8%，建议优化绩效激励结构', icon: TrendingUp, color: '#10b981' },
            { title: t('salary.ai.taxOptimize'), desc: '3名员工可增加专项附加扣除，预计月节税 ¥420', icon: ShieldCheck, color: '#8b5cf6' },
            { title: t('salary.ai.anomalyDetect'), desc: '销售部本月加班费异常增长 35%，建议关注', icon: Clock, color: '#f59e0b' },
          ].map((insight, i) => {
...
function SalaryCalculation() {
  ...
  const employees = useMemo(() => [
    { name: '张三', dept: '销售部', base: 8000, gross: 12800, deductions: 2180, net: 10620, status: t('salary.status.calculated'), statusColor: '#06b6d4' },
    { name: '李四', dept: '技术部', base: 12000, gross: 15600, deductions: 2890, net: 12710, status: t('salary.status.calculated'), statusColor: '#06b6d4' },
    { name: '王五', dept: '运营部', base: 9000, gross: 11200, deductions: 1750, net: 9450, status: t('salary.status.approved'), statusColor: '#10b981' },
    { name: '赵六', dept: '管理层', base: 15000, gross: 22500, deductions: 4820, net: 17680, status: t('salary.status.approved'), statusColor: '#10b981' },
    { name: '孙七', dept: '销售部', base: 8000, gross: 9600, deductions: 1420, net: 8180, status: t('salary.status.paid'), statusColor: '#8b5cf6' },
  ], [t])
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: t('salary.action.batchCalc'), desc: '全员月度薪资计算', icon: Calculator, color: '#8b5cf6' },
          { label: t('salary.action.approve'), desc: '审批薪资发放', icon: UserCheck, color: '#10b981' },
          { label: t('salary.action.export'), desc: '导出银行打款文件', icon: Download, color: '#06b6d4' },
        ].map((action, i) => {
      ...
      <NeonCard color="#8b5cf6" hoverable={false}>
        <h3 className="text-xs text-white/40 mb-4 uppercase tracking-wider">{t('salary.employeeList')}</h3>
        {/* Table Header */}
        <div className="grid grid-cols-7 gap-2 pb-2 mb-2" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          {['姓名', '部门', '基本工资', '应发合计', '扣减合计', '实发金额', '状态'].map((h, i) => (
            <span key={i} className="text-[10px] text-white/25 uppercase tracking-wider">{h}</span>
          ))}
        </div>
        {employees.map((emp, i) => (
          <div key={i} className="grid grid-cols-7 gap-2 py-2 items-center" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
            ...
            <span className="text-xs" style={{ color: '#10b981' }}>¥{emp.gross.toLocaleString()}</span>
            <span className="text-[10px]" style={{ color: '#ef4444' }}>-¥{emp.deductions.toLocaleString()}</span>
            <span className="text-xs font-medium" style={{ color: '#3b82f6', textShadow: '0 0 6px rgba(59,130,246,0.3)' }}>¥{emp.net.toLocaleString()}</span>
            <span className="text-[10px] px-1.5 py-0.5 rounded inline-block text-center" style={{ background: `${emp.statusColor}10`, color: emp.statusColor, border: `1px solid ${emp.statusColor}20` }}>{emp.status}</span>
          </div>
        ))}
      </NeonCard>
    </div>
...
function TaxManagement() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <NeonCard key={i} color={s.color}>
            ...
            <p className="text-lg" style={{ color: s.color, textShadow: `0 0 10px ${s.color}50` }}>{s.value}</p>
          </NeonCard>
        ))}
      </div>
      ...
      {/* Tax Bracket Table */}
      <NeonCard color="#8b5cf6" hoverable={false}>
        <h3 className="text-xs text-white/40 mb-4 uppercase tracking-wider">{t('salary.tax.brackets')}</h3>
        <div className="grid grid-cols-5 gap-2 pb-2 mb-2" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          {['级数', '应纳税所得额', '税率', '速算扣除数', '人数'].map((h, i) => (
            <span key={i} className="text-[10px] text-white/25 uppercase tracking-wider">{h}</span>
          ))}
        </div>
        {taxBrackets.map((bracket, i) => (
          <div key={i} className="grid grid-cols-5 gap-2 py-2 items-center" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
            <span className="text-xs text-white/50">{bracket.level}</span>
            <span className="text-[10px] text-white/40">{bracket.range}</span>
            <span className="text-xs" style={{ color: '#8b5cf6' }}>{bracket.rate}</span>
...
function AttendanceBoard() {
  ...
  const attendanceData = useMemo(() => [
    { name: '张三', dept: '销售部', workDays: 22, actual: 21, late: 1, leave: 1, overtime: 12, color: '#10b981' },
    { name: '李四', dept: '技术部', workDays: 22, actual: 22, late: 0, leave: 0, overtime: 24, color: '#06b6d4' },
    { name: '王五', dept: '运营部', workDays: 22, actual: 20, late: 2, leave: 2, overtime: 8, color: '#8b5cf6' },
    { name: '赵六', dept: '管理层', workDays: 22, actual: 22, late: 0, leave: 0, overtime: 6, color: '#f59e0b' },
    { name: '孙七', dept: '销售部', workDays: 22, actual: 19, late: 3, leave: 3, overtime: 16, color: '#ef4444' },
  ], [])
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Attendance Table */}
      <NeonCard color="#8b5cf6" hoverable={false}>
        <h3 className="text-xs text-white/40 mb-4 uppercase tracking-wider">{t('salary.att.summary')}</h3>
        <div className="grid grid-cols-7 gap-2 pb-2 mb-2" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        {attendanceData.map((emp, i) => (
          <div key={i} className="grid grid-cols-7 gap-2 py-2 items-center" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
            ...
            <span className="text-xs" style={{ color: emp.actual === emp.workDays ? '#10b981' : '#f59e0b' }}>{emp.actual}天</span>
            <span className="text-[10px]" style={{ color: emp.late > 0 ? '#f59e0b' : 'rgba(255,255,255,0.3)' }}>{emp.late}次</span>
            <span className="text-[10px]" style={{ color: emp.leave > 0 ? '#06b6d4' : 'rgba(255,255,255,0.3)' }}>{emp.leave}天</span>
            <span className="text-[10px]" style={{ color: emp.overtime > 20 ? '#ef4444' : '#8b5cf6' }}>{emp.overtime}h</span>
          </div>
        ))}
      </NeonCard>
    </div>
