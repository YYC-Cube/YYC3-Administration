...
function saveModels(models: AIModel[]) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(models)); } catch { /* ignore */ }
}
...
export function AIModelProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <AIModelContext.Provider value={{
      modelSettingsOpen, openModelSettings, closeModelSettings,
      aiModels, addAIModel, removeAIModel, updateAIModel, activateAIModel,
      activeModelId,
    }}>
      {children}
