...
import {
  AlertTriangle, AlignLeft, ArrowDownToLine, BookOpen,
  Bug, Check, Copy, FileCode, Loader2,
  MessageSquare, RefreshCw, Sparkles, TestTube, Wand2,
  Zap,
} from "lucide-react";
...
export const QUICK_ACTIONS: QuickAction[] = [
  { id: "copy", label: "复制", icon: Copy, color: "#6b7280", description: "复制到剪贴板", isAI: false },
  { id: "format", label: "格式化", icon: AlignLeft, color: "#3b82f6", description: "格式化代码", isAI: false },
  { id: "explain", label: "解释", icon: MessageSquare, color: "#a78bfa", description: "AI 代码解释", isAI: true },
  { id: "refactor", label: "重构", icon: RefreshCw, color: "#22c55e", description: "AI 重构建议", isAI: true },
  { id: "test", label: "测试", icon: TestTube, color: "#f97316", description: "生成测试", isAI: true },
  { id: "docs", label: "文档", icon: BookOpen, color: "#eab308", description: "生成文档", isAI: true },
  { id: "debug", label: "调试", icon: Bug, color: "#ef4444", description: "查找问题", isAI: true },
  { id: "optimize", label: "优化", icon: Wand2, color: "#ec4899", description: "AI 优化", isAI: true },
];
...
export function buildActionPrompt(actionId: string, fileName: string, codeContent: string): ChatMessage[] {
...
export function EditorQuickActions({ tc, filePath, editorContentGetter, editorInsertRef }: EditorQuickActionsProps) {
  const { addAIMessage, aiProviderConfig } = usePanelStore();
  const [activeAction, setActiveAction] = useState<string | null>(null);
  const [result, setResult] = useState<{ action: string; content: string; isAI: boolean; latencyMs?: number; provider?: string; streaming?: boolean } | null>(null);
    ...
    if (action.isAI) {
      ...
      try {
        ...
        if (messages.length === 0) {
          setResult({ action: action.id, content: getMockResponse(action.id, fileName, content.length), isAI: false });
          setActiveAction(null);
          return;
        }
        ...
        let fullContent = "";
        setResult({ action: action.id, content: "", isAI: true, streaming: true, provider: aiProviderConfig.provider });
  ...
  return (
    <div>
      {/* 操作栏 */}
      <div className="flex items-center gap-0.5 px-2 py-1 overflow-x-auto border-b" style={{ borderColor: tc.borderSubtle, background: tc.bgCard }}>
        ...
        {QUICK_ACTIONS.map((action) => {
          ...
          return (
            <button key={action.id} onClick={() => executeAction(action)}
              ...
              style={{ color: isActive ? action.color : tc.textMuted, opacity: activeAction && !isActive ? 0.4 : 1 }}
              title={`${action.description}${action.isAI && isRealProvider ? ` (${aiProviderConfig.provider} 流式)` : action.isAI ? " (模拟)" : ""}`}>
              {isActive ? <Loader2 className="w-3 h-3 animate-spin" /> : isDone ? <Check className="w-3 h-3" style={{ color: "#22c55e" }} /> : <Icon className="w-3 h-3" />}
              {action.label}
              {action.isAI && isRealProvider && <Zap className="w-2 h-2 ml-0.5" style={{ color: "#f59e0b" }} />}
            </button>
          );
        })}
        ...
        {/* 运行中取消按钮 */}
        {activeAction && (
          <button onClick={handleCancel} className="text-[8px] px-1.5 py-0.5 rounded-md hover:bg-white/5 shrink-0 ml-auto"
            style={{ color: "#ef4444" }}>
            取消
          </button>
        )}
        ...
        {/* 提供商指示器 */}
        {!activeAction && (
          <span className="text-[7px] px-1.5 py-0.5 rounded-full ml-auto shrink-0"
            style={{ background: isRealProvider ? "rgba(245,158,11,0.08)" : "rgba(107,114,128,0.08)", color: isRealProvider ? "#f59e0b" : tc.textMuted }}>
      ...
      <AnimatePresence>
        {error && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            ...
            <div className="px-3 py-1.5 text-[9px] border-b flex items-center gap-2" style={{ borderColor: tc.borderSubtle, background: "rgba(239,68,68,0.05)" }}>
              <AlertTriangle className="w-3 h-3 shrink-0" style={{ color: "#ef4444" }} />
              <span style={{ color: "#ef4444" }}>{error}</span>
              <button onClick={() => setError(null)} className="shrink-0 text-[8px] px-1 rounded hover:bg-white/5 ml-auto" style={{ color: tc.textMuted }}>×</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {result && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            className="overflow-hidden">
            <div className="px-3 py-2 text-[9px] border-b flex items-start gap-2" style={{ borderColor: tc.borderSubtle, background: "rgba(255,255,255,0.02)" }}>
              <FileCode className="w-3 h-3 shrink-0 mt-0.5" style={{ color: tc.primary }} />
              <div className="flex-1 min-w-0">
                {/* AI 响应元信息 */}
                {result.isAI && result.provider && (
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[7px] px-1.5 py-0.5 rounded-full" style={{ background: "rgba(245,158,11,0.1)", color: "#f59e0b" }}>
                      {result.provider} {result.streaming ? "· 流式传输中..." : result.latencyMs ? `· ${result.latencyMs}ms` : ""}
                    </span>
                    {result.streaming && <Loader2 className="w-2.5 h-2.5 animate-spin" style={{ color: "#f59e0b" }} />}
                  </div>
                )}
                <div className="whitespace-pre-wrap max-h-48 overflow-y-auto" style={{ color: tc.textSecondary, lineHeight: "1.5" }}>
                  {result.content}
                  {result.streaming && <span className="inline-block w-1.5 h-3 ml-0.5 animate-pulse rounded-sm" style={{ background: tc.primary }} />}
                </div>
              </div>
              <div className="flex flex-col gap-1 shrink-0">
                ...
                <button onClick={() => setResult(null)} className="text-[8px] px-1 rounded hover:bg-white/5" style={{ color: tc.textMuted }}>×</button>
              </div>
            </div>
          </motion.div>
        )}
