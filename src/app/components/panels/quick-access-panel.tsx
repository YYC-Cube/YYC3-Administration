...
export function QuickAccessPanel({ tc }: { tc: ThemeColors }) {
  ...
  const handleOpen = useCallback((item: QuickAccessItem) => {
  ...
  const renderItem = (item: QuickAccessItem, showFavBtn: boolean = true) => {
    ...
    return (
      <div key={item.path} onClick={() => handleOpen(item)}
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[11px] truncate" style={{ color: tc.textPrimary }}>{item.name}</p>
          <p className="text-[8px] truncate" style={{ color: tc.textMuted }}>{item.path}</p>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <span className="text-[8px] opacity-0 group-hover:opacity-60 transition-opacity" style={{ color: tc.textMuted }}>{timeAgo(item.lastAccessed)}</span>
          {showFavBtn && (
            <button onClick={(e) => { e.stopPropagation(); toggleFavorite(item); }}
              className="opacity-0 group-hover:opacity-100 transition-opacity">
              <Heart className="w-3 h-3" style={{ color: isFav ? "#ef4444" : tc.textMuted, fill: isFav ? "#ef4444" : "none" }} />
            </button>
          )}
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center px-3 py-2 border-b" style={{ borderColor: tc.borderSubtle }}>
        <span className="text-[11px] uppercase tracking-wider" style={{ color: tc.textMuted }}>快捷访问</span>
      </div>
      <div className="flex-1 overflow-y-auto">
        {favoriteFiles.length > 0 && (
          <div className="py-2">
            <div className="flex items-center gap-1.5 px-3 mb-1">
              ...
              <span className="text-[9px] uppercase tracking-wider" style={{ color: tc.textMuted }}>收藏 ({favoriteFiles.length})</span>
            </div>
        ...
        {recentFiles.length > 0 && (
          <div className="py-2 border-t" style={{ borderColor: tc.borderSubtle }}>
            <div className="flex items-center gap-1.5 px-3 mb-1">
              ...
              <span className="text-[9px] uppercase tracking-wider" style={{ color: tc.textMuted }}>最近 ({recentFiles.length})</span>
            </div>
        ...
        {recentFiles.length === 0 && favoriteFiles.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 opacity-40">
            ...
            <span className="text-[10px]" style={{ color: tc.textMuted }}>暂无最近或收藏文件</span>
            <span className="text-[8px] mt-0.5" style={{ color: tc.textMuted }}>从文件浏览器打开文件后，将在此显示</span>
          </div>
        )}
      </div>
    </div>
