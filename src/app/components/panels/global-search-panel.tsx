...
import {
  File, FolderOpen, Hash, History, Loader2, Search, X, Zap,
} from "lucide-react";
...
export function GlobalSearchPanel({ tc }: { tc: ThemeColors }) {
  const { searchHistory, addSearchHistory, fileTree, selectFile, addRecentFile, setActivePanel } = usePanelStore();
    ...
    if (searchType === "files") {
      ...
      const fileResults: SearchResult[] = flatFiles
        ...
        .map((f) => ({
          ...
          type: "file" as const,
          title: f.name,
          description: f.type === "directory" ? "Directory" : `${f.language ?? f.name.split(".").pop()} file`,
          filePath: f.path,
          score: f.name.toLowerCase().startsWith(lowerQ) ? 0.95 : 0.7 + Math.random() * 0.2,
        }));

      setResults(fileResults.length > 0 ? fileResults : MOCK_SEARCH_RESULTS.files?.slice(0, 3).map((r) => ({ ...r, score: 0.4 })) ?? []);
    } else {
      ...
      setResults(filtered.length > 0 ? filtered : pool.slice(0, 3).map((r) => ({ ...r, score: 0.5 + Math.random() * 0.3 })));
    }
    setSearching(false);
  ...
  const tabs = [
    { key: "files" as const, label: "文件", count: searchType === "files" && results.length > 0 ? results.length : undefined },
    ...
    <div className="flex flex-col h-full">
      <div className="px-3 py-2 border-b" style={{ borderColor: tc.borderSubtle }}>
        <div className="relative">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5" style={{ color: tc.textMuted }} />
          <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="搜索文件、内容、符号..."
        ...
        <div className="flex gap-0.5 mt-2">
          {tabs.map((tab) => (
            <button key={tab.key} onClick={() => setSearchType(tab.key)}
              ...
              style={{ background: searchType === tab.key ? `${tc.primary}12` : "transparent", color: searchType === tab.key ? tc.primary : tc.textMuted }}>
              {tab.label}
              {tab.count !== undefined && <span className="text-[7px] px-1 rounded-full" style={{ background: `${tc.primary}15` }}>{tab.count}</span>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!searching && results.length > 0 && results.map((r) => {
          ...
          const fileIcon = isFileResult ? getFileIcon(r.title) : typeIcons[r.type] ?? typeIcons.file;
          const ResultIcon = isFileResult ? fileIcon.icon : (typeIcons[r.type]?.icon ?? File);
          const iconColor = isFileResult ? fileIcon.color : (typeIcons[r.type]?.color ?? "#6b7280");
          return (
            <div key={r.id} onClick={() => handleOpenResult(r)}
              className="px-3 py-2 border-b cursor-pointer transition-colors hover:bg-white/[0.02]" style={{ borderColor: tc.borderSubtle }}>
              <div className="flex items-start gap-2">
                <ResultIcon className="w-3.5 h-3.5 mt-0.5 shrink-0" style={{ color: iconColor }} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <p className="text-[11px] truncate" style={{ color: tc.textPrimary }}>{r.title}</p>
                    <span className="text-[8px] shrink-0 ml-1" style={{ color: tc.textMuted }}>{Math.round(r.score * 100)}%</span>
                  </div>
                  {r.description && <p className="text-[9px] truncate" style={{ color: tc.textMuted }}>{r.description}</p>}
                  <div className="flex items-center gap-1 mt-0.5">
                    <span className="text-[8px] truncate" style={{ color: tc.textMuted }}>{r.filePath}</span>
                    {r.line && <span className="text-[8px] shrink-0" style={{ color: tc.textMuted }}>:{r.line}</span>}
                  </div>
                  {r.match && <p className="text-[9px] mt-1 px-1.5 py-0.5 rounded font-mono" style={{ background: "rgba(255,255,255,0.03)", color: tc.textSecondary }}>{r.match}</p>}
                </div>
              </div>
            </div>
          );
        ...
        {!searching && !query && searchHistory.length > 0 && (
          <div className="px-3 py-2">
            <p className="text-[9px] uppercase tracking-wider mb-2" style={{ color: tc.textMuted }}>最近搜索</p>
            {searchHistory.map((h, i) => (
              <button key={i} onClick={() => setQuery(h)} className="w-full text-left flex items-center gap-2 px-2 py-1 rounded-md transition-colors hover:bg-white/5">
                <History className="w-3 h-3" style={{ color: tc.textMuted }} />
                <span className="text-[10px]" style={{ color: tc.textSecondary }}>{h}</span>
              </button>
            ))}
          </div>
        )}
        {!searching && !query && searchHistory.length === 0 && (
          <div className="px-3 py-4 space-y-2">
            <p className="text-[9px] uppercase tracking-wider" style={{ color: tc.textMuted }}>快捷操作</p>
            {[
        ...
        {!searching && query && results.length === 0 && (
          <div className="flex flex-col items-center justify-center py-8 opacity-40">
            ...
            <span className="text-[10px]" style={{ color: tc.textMuted }}>未找到结果</span>
          </div>
        )}
      </div>
    </div>
