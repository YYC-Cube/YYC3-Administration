...
import type { AIChatMessage, AIProviderConfig, FileNode, PanelType, QuickAccessItem } from "./panel-types";
  ...
  persist(
    (set, get) => ({
      ...
      toggleFolder: (id) => set((s) => ({
        expandedFolders: s.expandedFolders.includes(id) ? s.expandedFolders.filter((f) => f !== id) : [...s.expandedFolders, id],
      })),
      selectFile: (path) => set({ selectedFile: path }),
      addRecentFile: (item) => set((s) => ({
      ...
      toggleFavorite: (item) => set((s) => {
        ...
        return {
          favoriteFiles: exists ? s.favoriteFiles.filter((f) => f.path !== item.path) : [...s.favoriteFiles, { ...item, type: "favorite" }],
        };
      }),
      ...
      setAIProviderConfig: (config) => set((s) => ({ aiProviderConfig: { ...s.aiProviderConfig, ...config } })),
      setFileTree: (tree) => set({ fileTree: tree }),
      addFileNode: (parentPath, node) => set((s) => {
        const addToTree = (nodes: FileNode[]): FileNode[] => nodes.map((n) => {
      ...
      deleteFileNode: (path) => set((s) => {
        const removeFromTree = (nodes: FileNode[]): FileNode[] => nodes.filter((n) => n.path !== path).map((n) => n.children ? { ...n, children: removeFromTree(n.children) } : n);
        return { fileTree: removeFromTree(s.fileTree), selectedFile: s.selectedFile === path ? null : s.selectedFile };
      }),
      renameFileNode: (oldPath, newName) => set((s) => {
        const renameInTree = (nodes: FileNode[]): FileNode[] => nodes.map((n) => {
          if (n.path === oldPath) {
