import {
  Activity, BarChart3, Bell, Brain, Briefcase, CheckCircle2, ChevronRight,
  Clock, Edit3, Globe, Mail, MapPin, Save, Settings, Shield, Star,
  Target, TrendingUp, UserCircle, Users, X, Zap,
} from "lucide-react";
...
export function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "activity" | "stats" | "preferences">("overview");
  ...
  return (
    <div className="h-full overflow-y-auto p-6" style={{ scrollbarWidth: "none", animation: "spring-in 0.4s var(--spring-easing) both" }}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="tracking-wider flex items-center gap-3" style={{ color: tc.secondary, textShadow: `0 0 15px ${tc.alpha(tc.secondary, 0.5)}` }}>
            <UserCircle className="w-6 h-6" />
            个人中心
          </h2>
          <p className="text-xs mt-1 tracking-wider" style={{ color: tc.textMuted }}>Personal Center — 用户档案与使用统计</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-6">
        ...
        <NeonCard color={tc.secondary} hoverable={false}>
          <div className="text-center mb-4">
            ...
            >
              <span className="text-2xl" style={{ color: tc.textSecondary }}>{profile.name[0]}</span>
              {/* Online indicator */}
              <div
            ...
            <h3 style={{ color: tc.textPrimary }} className="mb-0.5">{profile.name}</h3>
            <p className="text-xs mb-2" style={{ color: tc.textMuted }}>{profile.email}</p>
            <span
          ...
          <div className="space-y-2.5 pt-3 border-t" style={{ borderColor: tc.borderSubtle }}>
            <InfoRow icon={Briefcase} label="部门" value={profile.department} color={tc.secondary} />
            ...
            <InfoRow icon={Clock} label="加入" value={formatDate(profile.joinDate)} color={tc.success} />
          </div>
        ...
        <div className="xl:col-span-2 grid grid-cols-2 sm:grid-cols-3 gap-4">
          {[
            { label: "活跃天数", value: `${usageStats.daysActive}`, icon: Clock, color: tc.primary, sub: "天" },
            { label: "连续登录", value: `${usageStats.loginStreak}`, icon: Zap, color: tc.success, sub: "天" },
            { label: "AI 交互", value: `${usageStats.aiInteractions.toLocaleString()}`, icon: Brain, color: tc.secondary, sub: "次" },
            { label: "表单提交", value: `${usageStats.formSubmissions}`, icon: Target, color: tc.accent, sub: "份" },
            { label: "活动记录", value: `${usageStats.totalActivities}`, icon: Activity, color: tc.highlight, sub: "条" },
            { label: "系统通知", value: `${usageStats.totalNotifications}`, icon: Bell, color: tc.muted, sub: "条" },
          ].map((stat, i) => {
            ...
            return (
              <NeonCard key={i} color={stat.color}>
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider mb-1" style={{ color: tc.textMuted }}>{stat.label}</p>
                    <p className="text-xl" style={{ color: stat.color, textShadow: `0 0 10px ${tc.alpha(stat.color, 0.5)}` }}>
                      {stat.value}
                      <span className="text-xs ml-1" style={{ color: tc.textMuted }}>{stat.sub}</span>
                    </p>
                  </div>
                  <div 
      ...
      {/* Tab Content */}
      {activeTab === "overview" && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-5" style={{ animation: "spring-in 0.3s var(--spring-easing) both" }}>
          {/* Bio Card */}
          <NeonCard color={tc.secondary} hoverable={false}>
            <h3 className="text-xs uppercase tracking-wider mb-3" style={{ color: tc.textMuted }}>个人简介 · Bio</h3>
            <p className="text-sm leading-relaxed" style={{ color: tc.textSecondary }}>{profile.bio}</p>
          </NeonCard>

          {/* Skills/Badges */}
          <NeonCard color={tc.success} hoverable={false}>
            <h3 className="text-xs uppercase tracking-wider mb-3" style={{ color: tc.textMuted }}>成就徽章 · Achievements</h3>
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: "AI 先锋", desc: "首次使用 AI 工具", icon: Brain, color: tc.primary, unlocked: true },
                { label: "数据大师", desc: "提交 50+ 表单", icon: BarChart3, color: tc.secondary, unlocked: usageStats.formSubmissions >= 5 },
                { label: "社交达人", desc: "管理 100+ 客户", icon: Users, color: tc.accent, unlocked: true },
                { label: "效率之星", desc: "连续登录 7 天", icon: Star, color: tc.success, unlocked: usageStats.loginStreak >= 7 },
                { label: "安全卫士", desc: "完成安全认证", icon: Shield, color: tc.highlight, unlocked: true },
                { label: "趋势猎手", desc: "查看 100+ 洞察", icon: TrendingUp, color: tc.muted, unlocked: true },
              ].map((badge, i) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      background: badge.unlocked ? tc.alpha(badge.color, 0.05) : tc.alpha(tc.textMuted, 0.01),
                  ...
                  >
                    ...
                    <p className="text-[9px]" style={{ color: tc.textMuted }}>{badge.desc}</p>
                  </div>
                );
              })}
            </div>
          ...
          <NeonCard color={tc.primary} hoverable={false} className="xl:col-span-2">
            <h3 className="text-xs uppercase tracking-wider mb-3" style={{ color: tc.textMuted }}>快捷操作 · Quick Actions</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                { label: "数据驾驶舱", icon: BarChart3, color: tc.primary, page: "dashboard" as const },
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <span className="text-xs" style={{ color: tc.textSecondary }}>{action.label}</span>
                    </div>
      ...
      {activeTab === "activity" && (
        <NeonCard color={tc.success} hoverable={false}>
          <h3 className="text-xs uppercase tracking-wider mb-4" style={{ color: tc.textMuted }}>最近活动 · Recent Activity</h3>
          <div className="space-y-2">
            {recentActivities.slice(0, 15).map((act, i) => (
              ...
              >
                <div className="w-2 h-2 rounded-full shrink-0" style={{ background: act.color, boxShadow: `0 0 4px ${act.color}` }} />
                <div className="flex-1 min-w-0">
                  <p className="text-xs truncate" style={{ color: tc.textSecondary }}>{act.action}</p>
                  <p className="text-[10px] truncate" style={{ color: tc.textMuted }}>{act.target}</p>
                </div>
                <span className="text-[9px] shrink-0" style={{ color: tc.textMuted }}>{formatTimeAgo(act.timestamp)}</span>
              </div>
            ))}
            {recentActivities.length === 0 && (
              <div className="text-center py-8">
                <Activity className="w-8 h-8 mx-auto mb-2" style={{ color: tc.textMuted }} />
                <p className="text-xs" style={{ color: tc.textMuted }}>暂无活动记录</p>
              </div>
            )}
          </div>
        </NeonCard>
      ...
      {activeTab === "stats" && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-5" style={{ animation: "spring-in 0.3s var(--spring-easing) both" }}>
          <NeonCard color={tc.primary} hoverable={false}>
            <h3 className="text-xs uppercase tracking-wider mb-4" style={{ color: tc.textMuted }}>使用频率 · Usage Frequency</h3>
              ...
              {[
                ...
                <div key={i}>
                  <div className="flex justify-between mb-1">
                    <span className="text-[10px]" style={{ color: tc.textMuted }}>{item.label}</span>
                    <span className="text-[10px]" style={{ color: item.color }}>{item.pct}%</span>
                  </div>
                  <div className="h-1.5 rounded-full" style={{ background: tc.alpha(tc.textMuted, 0.05) }}>
                    <div
          ...
          <NeonCard color={tc.secondary} hoverable={false}>
            <h3 className="text-xs uppercase tracking-wider mb-4" style={{ color: tc.textMuted }}>效能指标 · Performance</h3>
            <div className="space-y-4">
              {[
                { label: "响应效率", value: "98.2%", desc: "平均响应时间 < 2s", color: tc.success, icon: Zap },
                { label: "任务完成率", value: "94.7%", desc: "本月任务完成情况", color: tc.secondary, icon: Target },
                { label: "客户满意度", value: "4.8/5", desc: "最近 30 天评分", color: tc.accent, icon: Star },
                { label: "AI 辅助率", value: "87.3%", desc: "AI 协助完成的工作", color: tc.primary, icon: Brain },
              ].map((item, i) => {
                  ...
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className="text-xs" style={{ color: tc.textSecondary }}>{item.label}</span>
                        <span className="text-sm" style={{ color: item.color, textShadow: `0 0 6px ${tc.alpha(item.color, 0.4)}` }}>
                          {item.value}
                        </span>
                      </div>
                      <p className="text-[9px]" style={{ color: tc.textMuted }}>{item.desc}</p>
                    </div>
                  </div>
                );
              })}
      ...
      {activeTab === "preferences" && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-5" style={{ animation: "spring-in 0.3s var(--spring-easing) both" }}>
          <NeonCard color={tc.accent} hoverable={false}>
            <h3 className="text-xs uppercase tracking-wider mb-4" style={{ color: tc.textMuted }}>通知偏好 · Notification Preferences</h3>
            <div className="space-y-3">
              {[
                ...
                <PreferenceToggle key={i} label={pref.label} desc={pref.desc} defaultEnabled={pref.enabled} color={tc.accent} />
              ))}
            </div>
          </NeonCard>

          <NeonCard color={tc.secondary} hoverable={false}>
            <h3 className="text-xs uppercase tracking-wider mb-4" style={{ color: tc.textMuted }}>显示设置 · Display Settings</h3>
            <div className="space-y-3">
              {[
                ...
                <PreferenceToggle key={i} label={pref.label} desc={pref.desc} defaultEnabled={pref.enabled} color={tc.secondary} />
              ))}
            </div>
          </NeonCard>
        </div>
          ...
          >
            ...
            <div className="space-y-4">
              {[
                ...
                { key: "website" as const, label: "网站", icon: Globe, placeholder: "输入网站 URL" },
              ].map(field => {
                const Icon = field.icon;
                return (
                  <div key={field.key}>
                    <label className="text-[10px] mb-1 flex items-center gap-1" style={{ color: tc.textMuted }}>
              ...
              <div>
                <label className="text-[10px] mb-1 block" style={{ color: tc.textMuted }}>个人简介</label>
                <textarea
                  value={editForm.bio}
                  ...
                  onFocus={e => { e.currentTarget.style.borderColor = tc.borderActive; }}
                  onBlur={e => { e.currentTarget.style.borderColor = tc.borderDefault; }}
                />
              </div>
            </div>

            <div className="flex justify-end gap-3 mt-6 pt-4 border-t" style={{ borderColor: tc.borderSubtle }}>
              <button
                onClick={handleCancel}
...
function InfoRow({ icon: Icon, label, value, color }: { icon: typeof Mail; label: string; value: string; color: string }) {
  const tc = useThemeColors();
  return (
    <div className="flex items-center gap-2.5">
      <Icon className="w-3.5 h-3.5 shrink-0" style={{ color: tc.alpha(color, 0.6) }} />
      <span className="text-[10px] w-10 shrink-0" style={{ color: tc.textMuted }}>{label}</span>
      <span className="text-xs truncate" style={{ color: tc.textSecondary }}>{value}</span>
    </div>
  );
}

function PreferenceToggle({ label, desc, defaultEnabled, color }: { label: string; desc: string; defaultEnabled: boolean; color: string }) {
  const tc = useThemeColors();
  ...
  return (
    ...
    >
      <div>
        <p className="text-xs" style={{ color: tc.textSecondary }}>{label}</p>
        <p className="text-[9px]" style={{ color: tc.textMuted }}>{desc}</p>
      </div>
      <button
