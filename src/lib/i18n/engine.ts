...
export class I18nEngine {
  ...
  public t(key: string, params?: Record<string, string>): string {
    try {
      ...
      const { key: modifiedKey, params: modifiedParams } = this.plugins.executeBeforeTranslate(
        key,
        params,
      )
      ...
      if (value === undefined || value === modifiedKey) {
        const fallback =
          this.plugins.handleMissingKey(modifiedKey, this.state.locale) ??
  ...
  private resolveTranslation(key: string): string | undefined {
    ...
    let value: unknown =
      this.state.translations[this.state.locale] ?? this.state.translations[DEFAULT_LOCALE]
    ...
    if (value === undefined && this.state.locale !== DEFAULT_LOCALE) {
  ...
  private handleError(error: Error, context: { key: string; locale: Locale }): void {
