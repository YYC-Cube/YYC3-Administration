...
export class ICUCompiler {
  ...
  private compileDate(
    ...
    try {
      return new Intl.DateTimeFormat(ctx.locale, { dateStyle: node.format ?? 'medium' }).format(
        value,
      )
    } catch {
  ...
  private compileTime(
    ...
    try {
      return new Intl.DateTimeFormat(ctx.locale, { timeStyle: node.format ?? 'medium' }).format(
        value,
      )
    } catch {
